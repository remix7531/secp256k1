(** * theory.modinv.divsteps.theory: correctness of the [process_divstep] convex-hull abstraction. *)
(** Copyright (C) 2026 remix7531
    Adapted from sipa/safegcd-bounds coq/divsteps/divsteps_theory.v
...
Require Import secp256k1.theory.modinv.divsteps.sound.

(* ================================================================= *)
(** ** Reduction and list-partition lemmas. *)

(** [Dred] does not change the value denoted: [Dred x] is [Deq] to [x]. *)
...
(** [DDSetProperties] brings the standard ordered-set reasoning lemmas (induction, [Add]) for [DDSet]. *)
Module DDSetProperties := MSetProperties.OrdProperties DDSet.

(** [Qsum] splits additively across a [partition]: the whole sum is the sum of the two partition halves. *)
...
(** [Qcombine] splits additively across a [partition], mirroring [Qsum_map_partition] for weighted points. *)
...
(** Every element landing in the first half of a [partition f] satisfies [f]. *)
...
(** Every element landing in the second half of a [partition f] fails [f]. *)
...
(* ================================================================= *)
(** ** Nested / nonempty convex-hull membership. *)

(** A point in the hull of [DDSet.add a s] is either [a] itself or a convex combination of [a] with a hull point of [s]. *)
...
(** If every point of [s1] lies (up to [QQeq]) in the hull of [s2], then the whole hull of [s1] lies in the hull of [s2]. *)
...
(** Replacing [s] by its [convex_hull] inside [DDSet.add q] does not shrink the set's own convex hull. *)
Lemma in_convex_hull_convex_hull : forall s q p,
  in_convex_hull p (DDSet.add q s) ->
  in_convex_hull p (DDSet.add q (convex_hull s)).
Proof.
  intros s q p H.
  ...
  - assert (H : in_convex_hull r (convex_hull s)).
    ...
    apply QQavg_in_convex_hull;auto.
    + apply in_in_convex_hull;apply DDSet.add_spec;left;reflexivity.
    + apply in_convex_hull_subset with (convex_hull s); auto.
      apply DDSetProperties.P.subset_add_2.
      apply DDSetProperties.P.subset_refl.
Qed.

(* ================================================================= *)
(** ** Per-delta state lookup -- [State_lookup]. *)

(** [State_lookup i m] is the point set tracked at key [i], or [DDSet.empty] if [m] tracks nothing there. *)
...
(** [State_join] never drops an existing point: anything in [State_lookup delta m] survives the join. *)
...
(** A point already in [DDSet.add q s] survives into [State_lookup delta (State_from_list l)] whenever [(delta, s)] occurs in [l]. *)
Lemma State_lookup_from_list : forall l q p delta s,
 DDSet.In p (DDSet.add q s) ->
 In (delta, s) l ->
 DDSet.In p (DDSet.add q (State_lookup delta (State_from_list l))).
Proof.
  intros l q p delta s Hps Hsl.
  rewrite DDSet.add_spec in *.
  destruct Hps as [Hps|Hps];[left;assumption|right].
  unfold State_from_list.
...
(** The convex-hull membership version of [State_lookup_from_list]: hull membership in [DDSet.add q s] survives into [State_from_list l]. *)
Lemma State_in_convex_hull_from_list1 : forall l q p delta s,
 in_convex_hull p (DDSet.add q s) ->
 In (delta, s) l ->
 in_convex_hull p (DDSet.add q (State_lookup delta (State_from_list l))).
Proof.
  intros l q p delta s [c [Hps1 [Hps2 [Hps3 Hps4]]]] Hsl.
  exists c;split;[|repeat split;auto].
  intros q' p' Hqp'.
  destruct (Hps1 q' p' Hqp') as [Hps11 Hps12].
  split; eauto using State_lookup_from_list.
Qed.

(* ================================================================= *)
(** ** Linear divstep transforms as 2x2 matrices over [D]. *)

(** [Matrix] is a 2x2 matrix of dyadic rationals, [(a,b,c,d)] read row-major. *)
Definition Matrix : Set := D * D * D * D.
(** [even_matrix] is the linear map underlying [even_trans]. *)
Definition even_matrix : Matrix :=
  (Dhalf 1, 0:D, 0:D, 1:D)%Z.
(** [odd_pos_matrix] is the linear map underlying [odd_pos_trans]. *)
Definition odd_pos_matrix : Matrix :=
  (Dhalf 1, Dhalf (-1), 1:D, 0:D)%Z.
(** [odd_nonpos_matrix] is the linear map underlying [odd_nonpos_trans]. *)
Definition odd_nonpos_matrix : Matrix :=
  (Dhalf 1, Dhalf 1, 0:D, 1:D)%Z.
(** [linear_d_of m p] applies the 2x2 matrix [m] to the dyadic point [p]. *)
Definition linear_d_of (m : Matrix) (p : DD) : DD :=
  let '(a,b,c,d) := m in
  (Dadd (Dmult a (fst p)) (Dmult b (snd p))
  ,Dadd (Dmult c (fst p)) (Dmult d (snd p))).
(** [linear_q_of] is [linear_d_of]'s counterpart over rational points [QQ]. *)
Definition linear_q_of (m : Matrix) (p : QQ) : QQ :=
  let '(a,b,c,d) := m in
  (a * (fst p) + b * (snd p), c * (fst p) + d * (snd p)).

(** [even_trans] is exactly the linear map [even_matrix]. *)
Lemma even_trans_matrix : forall p,
  DDeq (even_trans p) (linear_d_of even_matrix p).
Proof.
  intros [x y].
  unfold DDeq; simpl.
...
(** [odd_pos_trans] is exactly the linear map [odd_pos_matrix]. *)
Lemma odd_pos_trans_matrix : forall p,
  DDeq (odd_pos_trans p) (linear_d_of odd_pos_matrix p).
Proof.
  intros [x y].
...
(** [odd_nonpos_trans] is exactly the linear map [odd_nonpos_matrix]. *)
Lemma odd_nonpos_trans_matrix : forall p,
  DDeq (odd_nonpos_trans p) (linear_d_of odd_nonpos_matrix p).
Proof.
  intros [x y].
...
(** [linear_d_of m] respects [DDeq]: it sends equal dyadic points to equal dyadic points. *)
Lemma linear_d_of_morph : forall m q1 q2, 
  DDeq q1 q2 -> DDeq (linear_d_of m q1) (linear_d_of m q2).
Proof.
...
(** [DDSet_map trans] contains [trans p] for every [p] in the source set, given [trans] respects [DDeq]. *)
...
Proof.
  ...
  { rewrite SetoidList.InA_nil in Hps;elim Hps. }
  simpl.
...
(** Mapping a hull by a linear transform [trans] (matrix [m]) carries hull membership of [p] to hull membership of its image [q]. *)
Lemma in_convex_hull_map_ext : forall trans m p q s,
 (forall p, DDeq (trans p) (linear_d_of m p)) ->
 QQeq (linear_q_of m p) q ->
...
Proof.
  ...
  rewrite <-!Deq_Q.
  apply (linear_d_of_morph m _ _ Hq).
  exists (map (fun cp => (fst cp, linear_d_of m (snd cp))) l);split;[|split].
  - intros q [f g] H.
    ...
    change (DDOrder.eq (trans y) (linear_d_of m y)) in trans_ext.
    rewrite <- trans_ext.
    split;auto.
    rewrite DDSet.add_spec in *.
...
(* ================================================================= *)
(** ** Integer-halving helpers and the narrowing test -- [in_narrow]. *)

(** Integer division by 2 agrees with rational division by 2 on even [a]. *)
...
(** The [D] and [Q] embeddings of an integer [a] agree. *)
...
(** If [s] is [narrow] for modulus [M] and the scaled point [(g/M, f/M)] lies in its hull, then [g] must be 0. *)
...
(* ================================================================= *)
(** ** The abstraction invariant [in_State] and its preservation under [step]. *)

(** [in_State M ds s] is the abstraction relation: either [ds] has already converged ([g] = 0), or [ds]'s scaled point lies in the hull [s] tracks at its [delta]. *)
Definition in_State (M : Z) (ds : divsteps.State) (s : State) : Prop :=
  divsteps.g ds = 0%Z \/
  in_convex_hull (divsteps.g ds / M, divsteps.f ds / M)
   (DDSet.add (0:D,0:D)%Z (State_lookup (divsteps.delta ds) s)).

(** [in_State] is preserved by stepping both sides together: [divsteps.step] on the state and [process_divstep] on its hull abstraction. *)
Lemma in_State_step : forall M ds st, (0 < M)%Z -> Z.Odd (divsteps.f ds) ->
 in_State M ds st -> in_State M (divsteps.step ds) (process_divstep M st).
Proof.
  intros M [delta f g] st HM Hf [Hst|Hst];[left; auto using divsteps.zero_step|].
  rewrite <- Zodd_equiv in Hf.
  simpl in *.
  set (ds' := divsteps.step _).
  pose (delta' := divsteps.delta ds').
  pose (f' := divsteps.f ds').
  pose (g' := divsteps.g ds').
  unfold process_divstep.
  set (fm2 := fun kv => _).
  set (fm1 := fun kv => _).
  set (st1 := State_from_list (flat_map fm1 _)).
  set (st2 := State_from_list (flat_map fm2 _)).
  case (Z.eq_dec g 0) as [Hg0|Hg0].
  { left;unfold ds'; auto using divsteps.zero_step. }
  ...
  apply State_in_convex_hull_from_list1 with (DDSet_map divstep_trans (snd x)).
  - apply in_convex_hull_map_ext with divstep_matrix (g / M, f / M).
    ...
    + clear -Hf.
      unfold QQeq, g', f', ds', divsteps.step; simpl.
  ...
  - apply in_flat_map.
    ...
    unfold divsteps.step in ds'; cbn -[Z.ltb Z.add Z.sub] in *.
    rewrite <- Hx1.
  ...
  - clear s Hs.
    ...
    right.
    fold g' f' delta'.
    apply State_in_convex_hull_from_list1 with (convex_hull s).
    auto using in_convex_hull_convex_hull.
    apply in_flat_map.
    exists x.
    split; try tauto.
    destruct x as [x0 x1].
...
(** [ZMapProperties] brings the standard ordered-map reasoning lemmas for [ZMap]. *)
Module ZMapProperties := FMapFacts.OrdProperties ZMap.

(* ================================================================= *)
(** ** Main correctness -- [process_divstep_correct] and its gcd / inverse corollaries. *)

(** If iterating [process_divstep] [N] times from [state0] empties out, then the matching [divsteps.step] run from [init f g] has already converged ([g] = 0). *)
Theorem process_divstep_correct : forall N M f g,
  Z.Odd f ->
  (f <= M)%Z -> (0 <= g <= f)%Z -> 
  ZMap.Empty (N.iter N (process_divstep M) state0) ->
  divsteps.g (N.iter N divsteps.step (divsteps.init f g)) = 0%Z.
Proof.
  ...
  rewrite <- Zlt_Qlt.
  assumption.
  cut (in_State M (N.iter N divsteps.step (divsteps.init f g)) (N.iter N (process_divstep M) state0)).
  intros [H0|H0];auto.
  set (p := (_,_)) in *.
  set (st' := N.iter N divsteps.step _) in *.
  ...
  discriminate.
  clear H.
  cut (in_State M (divsteps.init f g) state0).
  intros H.
  apply proj2 with (Z.Odd (divsteps.f (N.iter N divsteps.step (divsteps.init f g)))).
  elim N using N.induction;[intros x y ->| |];try tauto.
  intros n.
  intros H0.
  rewrite !N.iter_succ.
  split;[apply divsteps.odd_step;tauto|].
...
(** Under [process_divstep_correct]'s hypotheses, the surviving [f] after [N] divsteps is a GCD of the original [f]/[g]. *)
Corollary process_divstep_gcd : forall N M f g,
  Z.Odd f ->
  (f <= M)%Z -> (0 <= g <= f)%Z -> 
  ZMap.Empty (N.iter N (process_divstep M) state0) ->
  Znumtheory.Zis_gcd f g
   (divsteps.f (N.iter N divsteps.step (divsteps.init f g))).
Proof.
  intros N M f g HM Hf Hg H.
  rewrite N2Nat.inj_iter.
  apply (divsteps.gcd_spec _ (divsteps.init f g)); auto.
  rewrite <- N2Nat.inj_iter.
  eapply process_divstep_correct; eauto.
Qed.

(** For coprime [f]/[g], the divstep run's final co-factor [d] times its final [f] inverts [g] modulo the original [f]. *)
Corollary process_divstep_inverse : forall N M f g,
  Z.Odd f ->
  (f <= M)%Z -> (0 <= g <= f)%Z -> 
  Znumtheory.rel_prime f g ->
  ZMap.Empty (N.iter N (process_divstep M) state0) ->
  let st' := N.iter N divsteps.step (divsteps.init f g) in
...
Proof.
  ...
  unfold st'; clear st'.
  rewrite N2Nat.inj_iter.
  apply divsteps.modulo_spec; auto using divsteps.modulo_init.
  rewrite <- N2Nat.inj_iter.
  eapply process_divstep_correct; eauto.
Qed.

(** The prime-modulus specialisation: for [g] = 0 the co-factor [d] is 0, and for [0 < g] it inverts [g] modulo the prime [f]. *)
Corollary process_divstep_prime_inverse : forall N M f g,
  Z.Odd f ->
  (g < f <= M)%Z ->
  Znumtheory.prime f ->
  ZMap.Empty (N.iter N (process_divstep M) state0) ->
...
Proof.
  ...
  - intros Hg.
    rewrite Hg in *.
    unfold st'.
    rewrite N2Nat.inj_iter, divsteps.d_zero_spec; rewrite Hg; reflexivity.
  - intros Hg.
    eapply (process_divstep_inverse N M); eauto with *.
