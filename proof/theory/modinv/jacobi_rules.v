(** * theory.modinv.jacobi_rules: the Jacobi-symbol rule characterisation. *)
(** Copyright (C) 2026 remix7531
    SPDX-License-Identifier: MIT *)

(** [JacobiRules] characterises the binary Euclidean algorithm
    [jacobi_symbol] by its base values and reduction rules.
    [jacobi_unique] establishes uniqueness for nonnegative numerators and
    positive odd denominators. [jacobi_pm1] gives a sign for coprime inputs.

    The two-supplement uses a reduced numerator, [0 <= a] and [2 * a < n].
    Reciprocity applies to all positive odd inputs, including noncoprime
    ones. Restricting it to coprime inputs would leave [J 3 9] unconstrained
    and invalidate uniqueness. *)
