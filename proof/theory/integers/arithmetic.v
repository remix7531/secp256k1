(** * theory.integers.arithmetic: pure Z lemmas for multi-limb arithmetic.
    ...
    - Residue window placement for a small-magnitude signed value
      ([small_abs_mod_window]).
    ...
    Canonical carry idiom: NEW work uses this file's carry/borrow
    vocabulary -- [arith_div_add_carry] / [arith_div_sub_borrow] (with
    the residue-comparison carry test) and their three-term sibling
    [div_sum_carry_split].  The frozen vendored [theory.extra_math]
    carries a parallel family ([div_add_carry] / [div_sub_borrow] /
    [mod_add_carry] / [mod_sub_borrow], phrased on [Z.b2z] of the
    wrapped-sum test and on [Z.shiftr]); it serves the vendored modinv
    (safegcd) port and is neither extended nor deduped against -- the
    two families intentionally coexist with distinct names and distinct
    statements. *)
...
(* ================================================================= *)
(** ** Limb evaluation and extraction -- [eval2] / [eval4] / [eval8] / [limb]. *)
...
(** Little-endian 5-limb evaluation (the base-[2^52] field representation). *)
Definition eval5 (B : Z) (a0 a1 a2 a3 a4 : Z) : Z :=
...
(** Limb-wise addition adds evaluations -- the algebraic core of the carry-free
    field linear ops ([fe_add] / [fe_negate] / [fe_mul_int]). *)
Lemma eval5_add : forall B a0 a1 a2 a3 a4 b0 b1 b2 b3 b4,
  eval5 B (a0 + b0) (a1 + b1) (a2 + b2) (a3 + b3) (a4 + b4)
  = eval5 B a0 a1 a2 a3 a4 + eval5 B b0 b1 b2 b3 b4.
Proof. intros. unfold eval5. ring. Qed.
...
(* ================================================================= *)
(** ** Power-expansion rewrite database -- [B^k] to [B*B*...*B] ([expand_pow]). *)
...
(* ================================================================= *)
(** ** Reconstruction lemmas -- [eval4_limbs] / [limbs_eval4] / [eval8_limbs]. *)
...
(* ================================================================= *)
(** ** Congruence lemmas -- [eval4]/[eval8] respect pointwise digit equality. *)
...
(* ================================================================= *)
(** ** Range and last-limb bounds -- [evalN_bound] / [evalN_mul_range] / [last_limb]. *)
...
(* ================================================================= *)
(** ** Injectivity lemmas -- [eval4_injective] / [eval8_injective] via digit peeling. *)
...
(* ================================================================= *)
(** ** Digit-uniqueness lemmas -- [evalN_digit_unique] / [limbs4_zero_iff]. *)
...
(** A value in [[0, B^4)] is zero iff all four of its base-[B] limbs are
    zero.  Used by the scalar limb-comparison predicates ([is_zero], and
    the comparison helpers that follow the same OR-of-limbs idiom). *)
Lemma limbs4_zero_iff : forall B x,
  B > 1 -> 0 <= x < B^4 ->
  (limb B x 0 = 0 /\ limb B x 1 = 0 /\ limb B x 2 = 0 /\ limb B x 3 = 0)
    <-> x = 0.
Proof.
  intros B x HB Hx.
  split.
  - (* all limbs zero: reconstruct x from its (zero) limbs *)
    intros [L0 [L1 [L2 L3]]].
    pose proof (eval4_limbs B x HB Hx) as Heval.
    rewrite L0, L1, L2, L3 in Heval.
    unfold eval4 in Heval.
    lia.
  - (* x = 0: every limb of 0 is 0 *)
    intros Hx0.
    subst x.
    unfold limb.
    rewrite !Zdiv.Zdiv_0_l, Zmod_0_l.
    auto.
Qed.
...
(* ================================================================= *)
(** ** Carry/borrow bridging lemmas -- [mod_add_residues_eq] /
       [div_sum_carry_split] / [arith_div_add_carry] / [arith_div_sub_borrow] /
       [div_mod_unique] / [low_digits_mod_sq]. *)

(** Pre-reducing the addends modulo [B] does not change a sum modulo
    [B]: replacing [y], [z] by their residues [y mod B], [z mod B]
    inside [(x + _ + _) mod B] leaves the result unchanged. *)
Lemma mod_add_residues_eq : forall B x y z,
  B > 0 ->
...
(** Carry of a three-term sum: dividing [x + y + z] by [B] equals the
    sum of the two high parts [y / B + z / B] plus the carry out of the
    low residues [(x + y mod B + z mod B) / B]. *)
Lemma div_sum_carry_split : forall B x y z,
  B > 0 ->
...
(** Carry recombination for limb-wise addition: the high limb of a sum
    is the sum of the high limbs plus the carry out of the low limbs.
    Holds for arbitrary integers [R], [P] (the high limbs may be
    negative) because both residues [R mod M], [P mod M] are
    nonnegative.  Specialised to [M = 2^64] by the [i128] signed-add
    proofs ([i128_accum_mul]). *)
Lemma arith_div_add_carry : forall R P M,
  0 < M ->
  (R + P) / M =
  R / M + P / M +
  (if R mod M + P mod M <? M then 0 else 1).
Proof.
  intros R P M HM.
  assert (Hid : (R + P) / M =
    R / M + P / M + (R mod M + P mod M) / M).
  { rewrite (Z.div_mod R M) at 1 by lia.
    rewrite (Z.div_mod P M) at 1 by lia.
    replace (M * (R / M) + R mod M + (M * (P / M) + P mod M))
      with ((R / M + P / M) * M + (R mod M + P mod M)) by ring.
    rewrite Z.div_add_l by lia.
    ring. }
  rewrite Hid.
  f_equal.
  assert (Hr : 0 <= R mod M < M) by (apply Z.mod_pos_bound; lia).
  assert (Hp : 0 <= P mod M < M) by (apply Z.mod_pos_bound; lia).
  destruct (R mod M + P mod M <? M) eqn:Hc.
  - apply Z.ltb_lt in Hc.
    apply Z.div_small.
    lia.
  - apply Z.ltb_ge in Hc.
    symmetry.
    apply Z.div_unique with (r := R mod M + P mod M - M); lia.
Qed.

(** Borrow recombination for limb-wise subtraction: subtracting [Q] from
    [R] limb-by-limb, the high limb is [R/M - Q/M] decremented by the
    borrow [R mod M < Q mod M], and that equals the high limb [(R-Q)/M]
    of the true difference.  The subtractive sibling of [arith_div_add_carry];
    specialised to [M = 2^64] by the [i128] signed-subtract proofs
    ([i128_dissip_mul]). *)
Lemma arith_div_sub_borrow : forall R Q M,
  0 < M ->
  R / M - (Q / M + Z.b2z (R mod M <? Q mod M)) = (R - Q) / M.
Proof.
  intros R Q M HM.
  pose proof (Z.div_mod R M ltac:(lia)) as HR.
  pose proof (Z.div_mod Q M ltac:(lia)) as HQ.
  pose proof (Z.mod_pos_bound R M HM) as HRb.
  pose proof (Z.mod_pos_bound Q M HM) as HQb.
  destruct (R mod M <? Q mod M) eqn:Hc; simpl Z.b2z.
  - (* borrow taken: low residue underflows, quotient drops by 1 *)
    apply Z.ltb_lt in Hc.
    apply (Z.div_unique (R - Q) M (R / M - (Q / M + 1))
                        (R mod M - Q mod M + M)); [left; lia | nia].
  - (* no borrow: low residue stays in range *)
    apply Z.ltb_ge in Hc.
    apply (Z.div_unique (R - Q) M (R / M - (Q / M + 0))
                        (R mod M - Q mod M)); [left; lia | nia].
Qed.

(** Limb-decomposition uniqueness: a value is determined by its quotient
    [_ / M] and residue [_ mod M].  This is the [Z.div_mod] identity, so
    no range hypothesis is needed beyond [M <> 0].  The pure core of the
    [i128] limb-equality tests ([i128_eq_var], [i128_check_pow2]),
    specialised to [M = 2^64]. *)
Lemma div_mod_unique : forall a b M,
  M <> 0 ->
  a / M = b / M ->
  a mod M = b mod M ->
  a = b.
Proof.
  intros a b M HM Hq Hr.
  rewrite (Z.div_mod a M) by lia.
  rewrite (Z.div_mod b M) by lia.
  rewrite Hq, Hr.
  reflexivity.
Qed.

(** Two-digit reconstruction: the low two base-[B] digits of [x] --
    [x mod B] and [(x / B) mod B] -- reassemble to [x mod (B * B)],
    i.e. [x] modulo [B] squared. *)
Lemma low_digits_mod_sq : forall B x,
  B > 0 ->
...
(* ================================================================= *)
(** ** Schoolbook 2x2 multiplication correctness -- [schoolbook_mul_2x2]. *)
...
(* ================================================================= *)
(** ** Schoolbook 4x4 multiplication correctness -- [schoolbook_mul_4x4]. *)
...
(* ================================================================= *)
(** ** Carry-chain identities for 4-limb ripple addition -- [reduce_carry_chain] / [add4_carry_chain]. *)
...
Lemma reduce_carry_chain :
  forall B d0 d1 d2 d3 c0 c1 c2 (ov : Z),
    ...
    let val := eval4 B d0 d1 d2 d3 in
    ...
    let t3 := t2 / B + d3 in
    let r_z := eval4 B (t0 mod B) (t1 mod B) (t2 mod B) (t3 mod B) in
    let hi := t3 / B in
    r_z + hi * B4 = val + ov * C
    /\ 0 <= r_z < B4
    /\ 0 <= hi <= 1.
Proof.
  ...
  assert (Hstep0 : val + ov * C = t0 + (d1 + ov * c1) * B + (d2 + ov * c2) * B2 + d3 * B3)
    by (subst val C t0 B2 B3; unfold eval4; ring).
  ...
  assert (Hr_z_eq : r_z = m0 + m1 * B + m2 * B2 + m3 * B3)
    by (subst r_z m0 m1 m2 m3 B2 B3; unfold eval4; ring).
...
(** Four-limb ripple addition where *every* limb (including limb 3) gets
    its own constant addend [c_i] -- the full 4x64-bit add shape, as
    opposed to [reduce_carry_chain]'s three-constant [ov * C] folding.
    At each stage the low limb is extracted ([mod B]) and the carry is
    propagated ([/ B]); the four extracted limbs plus the final carry
    [hi] reconstruct exactly to [val + C].  Shared by [scalar_add],
    [scalar_negate], and [scalar_half] (all the same identity). *)
Lemma add4_carry_chain :
  forall B d0 d1 d2 d3 c0 c1 c2 c3,
    1 < B ->
    0 <= d0 < B -> 0 <= d1 < B -> 0 <= d2 < B -> 0 <= d3 < B ->
    0 <= c0 -> 0 <= c1 -> 0 <= c2 -> 0 <= c3 ->
    let B2 := B * B in let B3 := B * B * B in let B4 := B * B * B * B in
    let val := eval4 B d0 d1 d2 d3 in
    let C := eval4 B c0 c1 c2 c3 in
    0 <= val + C < 2 * B4 ->
    let t0 := d0 + c0 in
    let t1 := t0 / B + d1 + c1 in
    let t2 := t1 / B + d2 + c2 in
    let t3 := t2 / B + d3 + c3 in
    let r_z := eval4 B (t0 mod B) (t1 mod B) (t2 mod B) (t3 mod B) in
    let hi := t3 / B in
    r_z + hi * B4 = val + C
    /\ 0 <= r_z < B4
    /\ 0 <= hi <= 1.
Proof.
  intros B d0 d1 d2 d3 c0 c1 c2 c3 HB Hd0 Hd1 Hd2 Hd3 Hc0 Hc1 Hc2 Hc3
    B2 B3 B4 val C Hresult t0 t1 t2 t3 r_z hi.

  (* Non-negativity of intermediate accumulators *)
  assert (Ht0_nn : 0 <= t0) by (subst t0; lia).
  assert (Ht1_nn : 0 <= t1).
  { subst t1.
    assert (0 <= t0 / B) by (apply Z.div_pos; lia).
    lia. }
  assert (Ht2_nn : 0 <= t2).
  { subst t2.
    assert (0 <= t1 / B) by (apply Z.div_pos; lia).
    lia. }
  assert (Ht3_nn : 0 <= t3).
  { subst t3.
    assert (0 <= t2 / B) by (apply Z.div_pos; lia).
    lia. }
  ...
  (* t_{i+1} = q_i + d_{i+1} + c_{i+1} *)
  assert (Ht1_def : t1 = q0 + d1 + c1) by (subst t1 q0; ring).
  assert (Ht2_def : t2 = q1 + d2 + c2) by (subst t2 q1; ring).
  assert (Ht3_def : t3 = q2 + d3 + c3) by (subst t3 q2; ring).
  ...
  (* Telescoping: substitute div/mod at each round *)
  assert (Hstep0 : val + C =
    t0 + (d1 + c1) * B + (d2 + c2) * B2 + (d3 + c3) * B3)
    by (subst val C t0 B2 B3; unfold eval4; ring).
  assert (Hstep1 : val + C =
    m0 + t1 * B + (d2 + c2) * B2 + (d3 + c3) * B3)
    by (rewrite Hstep0, Ht0_eq, Ht1_def; subst B2 B3; ring).
  assert (Hstep2 : val + C =
    m0 + m1 * B + t2 * B2 + (d3 + c3) * B3)
    by (rewrite Hstep1, Ht1_eq, Ht2_def; subst B2 B3; ring).
  assert (Hstep3 : val + C =
    m0 + m1 * B + m2 * B2 + t3 * B3)
    by (rewrite Hstep2, Ht2_eq, Ht3_def; subst B2 B3; ring).
  ...
  (* Expand t3 = B * hi + m3 in the telescoped identity *)
  assert (Hstep4 : val + C = m0 + m1 * B + m2 * B2 + m3 * B3 + hi * B4)
    by (rewrite Hstep3, Ht3_eq; subst B2 B3 B4; ring).
...
(* ================================================================= *)
(** ** Modular folding lemmas -- [fold_sub_mod] / [cond_sub_mod]. *)
...
(* ================================================================= *)
(** ** Residue window placement -- [small_abs_mod_window]. *)

(** A signed value of magnitude [< B] reduces mod [M] (for [B <= M]) into
    one of the two "low" windows [[0, B)] or [(M - B, M)] -- the shape of a
    signed-representative postcondition disjunction. *)
Lemma small_abs_mod_window : forall v B M,
  0 < M -> B <= M -> Z.abs v < B ->
  v mod M < B \/ M - v mod M < B.
Proof.
  intros v B M HM HBM Hv.
  apply Z.abs_lt in Hv.
  destruct (Z_lt_le_dec v 0) as [Hneg | Hpos].
  - (* branch: v < 0 -- mod lands in the high window [v mod M = v + M] *)
    right.
    assert (Hvm : v mod M = v + M).
    { rewrite <- (Z.mod_add v 1 M) by lia.
      rewrite Z.mul_1_l.
      apply Z.mod_small.
      lia. }
    rewrite Hvm. lia.
  - (* branch: v >= 0 -- already in [[0, B)] *)
    left.
    rewrite Z.mod_small by lia.
    lia.
Qed.
