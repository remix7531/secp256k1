(** * theory.integers.extra_math: general arithmetic lemmas for the modinv proof. *)
(** Copyright (C) 2026 remix7531
    Ported from BlockstreamResearch/simplicity Coq/C/extraMath.v
...
(* ================================================================= *)
(** ** Generic helpers -- [f_if] / [div_bounds]. *)

(** Push a function through an [if]: [f (if b then x else y)]. *)
...
(** Bracket on [b / d] from a bracket on [b] scaled by [d]. *)
...
(* ================================================================= *)
(** ** Carry on addition -- [mod_add_carry] / [div_add_carry] / shiftr forms. *)

(** Carry-out of [a mod m + b mod m] as the wrap-around boolean. *)
...
(** Quotient of a sum splits into the per-term quotients plus the carry. *)
...
(** Carry on adding two [2^n]-reductions, expressed via [Z.shiftr]. *)
...
(** Shift of a sum splits into the per-term shifts plus the carry. *)
...
(* ================================================================= *)
(** ** Borrow on subtraction -- [mod_sub_borrow] / [div_sub_borrow] / shiftr forms. *)

(** Borrow-out of [a mod m - b mod m] as the negated underflow boolean. *)
...
(** Quotient of a difference splits into the per-term quotients minus the borrow. *)
...
(** Borrow on subtracting two [2^n]-reductions, expressed via [Z.shiftr]. *)
...
(** Shift of a difference splits into the per-term shifts minus the borrow. *)
...
(* ================================================================= *)
(** ** Right-shift bounds and small-value cases -- [shiftr_bounds] / [shiftr_small_*]. *)

(** [Z.shiftr (-1) n = -1] for nonnegative [n]. *)
...
(** Bracket on [Z.shiftr b n] from a bracket on [b] scaled by [2^n]. *)
...
(** A value in [0, 2^n) shifts right to zero. *)
...
(** [Z.shiftr a n = 0] iff [a] lies in [0, 2^n). *)
...
(** A value in [-2^n, 0) shifts right to -1. *)
...
(** [Z.shiftr a n = -1] iff [a] lies in [-2^n, 0). *)
...
(** A signed value in [-2^n, 2^n) is unchanged by shifting [m] bits further. *)
...
(* ================================================================= *)
(** ** Modular equality and uniqueness -- [shiftl_mod_eqb_unique] / [Z_mod_eq_bounded]. *)

(** [shiftl]/[mod] decomposition gives a unique pair, hence an [eqb] split. *)
...
(** Two values within a window of width [<= m] that agree mod [m] are equal. *)
...
(* ================================================================= *)
(** ** Bitwise or and log2 -- [Z_lor_pos] / [Z_log2_lt_pow2]. *)

(** [Z.lor] of a nonnegative with a positive is positive. *)
...
(** [a < 2^b] iff [Z.log2 a < b] (for positive [b]). *)
...
(* ================================================================= *)
(** ** Bit masks, [land], and [eqm] -- [Z_shiftr_ones] / [Z_land_ones_min] / [eqm_2_pow_le]. *)

(** Shifting a [Z.ones a] mask right by [n] yields [Z.ones (a - n)]. *)
...
(** [Z.land] of two masks is the mask of the shorter width. *)
...
(** Congruence mod [2^n] implies congruence mod the smaller [2^m]. *)
...
(** Commute a [mod 2^j] under a [Z.shiftr _ i] (widening the modulus by [i]). *)
...
(* ================================================================= *)
(** ** Comparison-to-Z -- [Z_of_comparison] / [comp_Z_of_comparison]. *)

(** Encode a [comparison] as the sign value -1 / 0 / 1. *)
...
(** Comparing [Z_of_comparison c] with 0 recovers [c]. *)
Lemma comp_Z_of_comparison c : Z_of_comparison c ?= 0 = c.
Proof.
  destruct c; reflexivity.
Qed.

(** [Z_of_comparison] is bounded by [-1, 1]. *)
...
(* ================================================================= *)
(** ** Count trailing zeros -- [Pos_ctz] / [Z_ctz] / [Z_ctz_*]. *)

(** Count trailing zeros of a positive (the [xO] depth). *)
...
(** Count trailing zeros of an integer (0 at [a = 0] by convention). *)
...
(** [Z_ctz] is nonnegative. *)
Lemma Z_ctz_non_neg a : 0 <= Z_ctz a.
Proof.
  destruct a; cbn; lia.
Qed.

(** [Pos_ctz] is bounded by the bit-width of its argument. *)
...
(** [Z_ctz a < n] for any nonzero [a] in [(-2^n, 2^n)]. *)
...
(** Every bit below [Z_ctz a] is clear. *)
...
(** The bit at [Z_ctz a] is set for nonzero [a]. *)
...
(* ================================================================= *)
(** ** Negation, testbit, and [a AND -a] -- [Z_neg_lnot] / [Z_testbit_neg_*] / [Z_x_and_opp_x]. *)

(** Two's-complement identity: [-a = Z.lnot a + 1]. *)
...
(** Low bits (up to [Z_ctz]) of [-a] agree with those of [a]. *)
...
(** Low bits (up to [Z_ctz]) of [-a] agree with those of [a], for any [a]. *)
...
(** High bits (above [Z_ctz]) of [-a] are the negation of those of [a]. *)
...
(** High bits (above [Z_ctz]) of [-a] are the negation of those of [a], for any [a]. *)
...
(** Isolate the lowest set bit: [a AND -a = 2^(Z_ctz a)]. *)
...
(* ================================================================= *)
(** ** [Z_ctz] vs prefix-of-zeros and [Z.lor] -- [Z_testbit_false_ctz] / [Z_ctz_lor_l]. *)

(** A zero prefix up to [i] forces [i < Z_ctz a] (for nonzero [a]). *)
...
(** [Z.lor] cannot increase the count of trailing zeros below the left operand. *)
