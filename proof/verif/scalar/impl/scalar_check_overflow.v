...
(* ================================================================= *)
(** ** secp256k1_scalar_check_overflow -- [a >= N]. *)
...
Lemma body_secp256k1_scalar_check_overflow:
  semax_body Vprog Gprog
    f_secp256k1_scalar_check_overflow spec_secp256k1_scalar_check_overflow.
Proof.
  start_function.
  ...
  (* Destruct all 6 zlt comparisons (d3<N_3, d2<N_2, N_2<d2, d1<N_1,
     ...
     Every branch takes the same closing chain, so it hangs off the
     destruct instead of re-selecting the branches. *)
  do 6 match goal with |- context [zlt ?a ?b] => destruct (zlt a b) end;
      ...
      try reflexivity;
      try discriminate;
      (* What is left: False from a contradictory combination of limb
