(** * vst.scalar.impl: INTERNAL funspecs for the scalar subsystem. *)
...
(** Mirrors [src/scalar_4x64_impl.h]: the funspecs here are for the [static]
    ...
    [inverse], [inverse_var]) is specified in [vst/scalar/contract.v], whose
    [Gprog_scalar_public] it joins; the internal ones below are listed in this
    file's [Gprog_scalar_impl].  [vst/gprog.v] concatenates the two. *)
...
Require Import secp256k1.theory.int128.acc.
...
(** These reference the [Signed62] representation ([vst/helper/signed62.v]); they
...
(* ================================================================= *)
(** ** The module's internal funspec list -- the [static] helpers of
    [scalar_4x64_impl.h], plus the two [scalar <-> signed62] converters that
    the safegcd drivers call. *)

Definition Gprog_scalar_impl : funspecs := [
