(** * vst.scalar.contract: funspecs for the PUBLIC scalar API ([src/scalar.h]). *)
...
Require Import secp256k1.vst.helper.signed62.
...
(** ([secp256k1_scalar_reduce_512] is internal -- see [vst/scalar/impl.v].) *)
...
(** ([shift_limb] is internal -- see [vst/scalar/impl.v].) *)
...
(* ================================================================= *)
(** ** GLV table views -- [c_lambda] / [c_minus_b1] / [c_minus_b2] / [c_g1] /
    [c_g2].

    The five 256-bit constants [secp256k1_scalar_split_lambda] reads out of
    global memory, as [UInt256]s so that [u256_at] can name them in [SEP].
    They are C-representation views of mathematical values
    ([Math.group]'s [secp256k1_lambda]) or in
    [theory/scalar/limbs.v] (the four basis constants), and exist only
    here because only this funspec and its body proof mention them. *)

(** [secp256k1_const_lambda]. *)
Lemma c_lambda_range : (0 <= secp256k1_lambda < 2^256)%Z.
Proof. unfold secp256k1_lambda; lia. Qed.

Definition c_lambda : UInt256 := mkUInt256 secp256k1_lambda c_lambda_range.

(** [minus_b1]. *)
Lemma c_minus_b1_range : (0 <= minus_b1_z < 2^256)%Z.
Proof. unfold minus_b1_z; lia. Qed.

Definition c_minus_b1 : UInt256 := mkUInt256 minus_b1_z c_minus_b1_range.

(** [minus_b2]. *)
Lemma c_minus_b2_range : (0 <= minus_b2_z < 2^256)%Z.
Proof. unfold minus_b2_z; lia. Qed.

Definition c_minus_b2 : UInt256 := mkUInt256 minus_b2_z c_minus_b2_range.

(** [g1]. *)
Lemma c_g1_range : (0 <= g1_z < 2^256)%Z.
Proof. unfold g1_z; lia. Qed.

Definition c_g1 : UInt256 := mkUInt256 g1_z c_g1_range.

(** [g2]. *)
Lemma c_g2_range : (0 <= g2_z < 2^256)%Z.
Proof. unfold g2_z; lia. Qed.

Definition c_g2 : UInt256 := mkUInt256 g2_z c_g2_range.
...
(** Public scalar API ([src/scalar.h]).  Both reference the [Signed62] /
    [make_modinfo] glue ([vst/helper/signed62.v]) and the constant
    [secp256k1_const_modinfo_scalar] holding [make_modinfo N]; the bodies are
    proved (in [vst/scalar/verif/scalar_inverse{,_var}.v]) against the modinv call
    ...
    ([Math.order_prime]'s [secp256k1_N_prime]), so every scalar is invertible (or 0). *)
...
(* ================================================================= *)
(** ** The module's funspec list -- the public [src/scalar.h] API.

    Includes the two modular-inverse entry points: they are scalar functions
    (declared above) that bridge into the safegcd drivers, so they belong to
    this module's list even though the drivers they call are modinv's. *)

Definition Gprog_scalar_public : funspecs := [
