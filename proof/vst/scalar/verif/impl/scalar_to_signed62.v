(** * vst.scalar.verif.impl.scalar_to_signed62: body proof for secp256k1_scalar_to_signed62 *)
...
(** Repack the scalar into 62-bit little-endian limbs; the stored result is
    [reprn 5 (scalar_val a)].  The spec + Gprog live in vst/modinv/contract.v (the
    Signed62 owner).
    ...
    sum mod 2^62 is [z mod 2^62]), both in theory/integers/limb_window.v.  The Int64
    glue stays local:
...
Proof.
  ...
  assert (HX : 0 <= scalar_val a < 2 ^ 256).
  { pose proof (scalar_range a).
    assert (secp256k1_N <= 2 ^ 256) by (unfold secp256k1_N; lia).
    lia. }
