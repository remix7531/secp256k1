...
(* ================================================================= *)
(** ** secp256k1_scalar_reduce_512 -- [r = l mod N] (three-stage fold). *)

(** Three-stage modular folding: 512 -> 385 -> 258 -> 256 bits. *)

(* ----------------------------------------------------------------- *)
(** *** Helper lemmas and tactics *)
(** The three lemmas below are file-local because they are tightly
    coupled to the concrete VST representation types used in this proof
    (acc_to_val, Int64.repr, uint256_to_val) and do not generalise to
    an abstract base-B statement that would belong in theory/.  They
    are not placed in tactics/ because they are not Ltac automation;
    they are propositional lemmas used via [apply] / [exact] inside
    this proof only. *)

(** [acc_init_eq] bridges the raw triple written by the acc-init
    sequence (c0 = v, c1 = 0, c2 = 0) to [acc_to_val (mkAcc v Hv)],
    exploiting v < 2^64 so the division and mod terms vanish. *)
...
(** [Int_repr_mod_Int64] lets us drop the [x mod Int64.modulus] that
    appears when VST normalises an Int64-width constant to a 32-bit
    [Int.repr]: [Int64.modulus = 2^64] divides [Int.modulus = 2^32]'s
    universe, so the two reprs agree.  Needed to discharge the cast
    side-condition in the final [forward_call_scalar_reduce]. *)
...
(** [uint256_from_limbs] converts a list of four [uint64_to_val]
    projections into the canonical [uint256_to_val (mkUInt256 r_z Hr)]
    form.  It is file-local because it depends on the concrete
    [uint256_to_val] definition (which unfolds to [limb]-based splits)
    and on [limbs_eval4] from the project theory layer.  Placing it
    in tactics/ would expose the representation type to the automation
    layer, which should remain rep-agnostic. *)
...
(* ----------------------------------------------------------------- *)
(** *** Stage lemmas *)
(** The lemmas below state, once each, the pure arithmetic of one folding
    stage: the carry bound shared by every extract round, and the modular
    identity each of the three stages establishes.  They are file-local
    because they name this function's stage structure (the [N_C] limbs, the
    385- and 258-bit intermediate layouts) rather than a reusable base-B
    fact; the generic facts they rest on ([reduce_carry_chain],
    [cond_sub_mod], [fold_sub_mod], [eval8_limbs]) already live in
    [theory/arithmetic] and [model/constants]. *)

(** [reduce_512_carry_ub] bounds a round's carry-out.  The carry is the
    accumulator shifted down one 64-bit limb, so an accumulator bounded by
    [2^64 * c + 2^64 - 1] leaves a carry of at most [c].  One lemma for all
    ten extract rounds of Stages 1 and 2. *)
Lemma reduce_512_carry_ub : forall a v c : Z,
  a = v / 2^64 ->
  v <= 2^64 * c + 2^64 - 1 ->
  a <= c.
Proof.
  intros a v c Ha Hv.
  rewrite Ha.
  apply (carry_div_ub_eq _ v _); lia.
Qed.

(** [reduce_512_stage1_mod] is Stage 1 (512 -> 385 bits): the six extract
    rounds telescope to [l_lo + l_hi * N_C], which is congruent mod [N] to
    the 8-limb input [lv] because [N_C = 2^256 - N].  [c_k] is the carry out
    of round [k] and [m_k] the limb it extracts. *)
Lemma reduce_512_stage1_mod :
  forall l0 l1 l2 l3 n0 n1 n2 n3 m0 m1 m2 m3 m4 m5
         c0 c1 c2 c3 c4 c5 lv : Z,
  eval8 (2^64) l0 l1 l2 l3 n0 n1 n2 n3 = lv ->
  m0 + c0 * 2^64 = l0 + n0 * N_C_0 ->
  m1 + c1 * 2^64 = c0 + l1 + n1 * N_C_0 + n0 * N_C_1 ->
  m2 + c2 * 2^64 = c1 + l2 + n2 * N_C_0 + n1 * N_C_1 + n0 ->
  m3 + c3 * 2^64 = c2 + l3 + n3 * N_C_0 + n2 * N_C_1 + n1 ->
  m4 + c4 * 2^64 = c3 + n3 * N_C_1 + n2 ->
  m5 + c5 * 2^64 = c4 + n3 ->
  (m0 + m1 * 2^64 + m2 * 2^128 + m3 * 2^192 + m4 * 2^256
   + m5 * 2^320 + c5 * 2^384) mod secp256k1_N = lv mod secp256k1_N.
Proof.
  intros l0 l1 l2 l3 n0 n1 n2 n3 m0 m1 m2 m3 m4 m5
    c0 c1 c2 c3 c4 c5 lv Heval Hr0 Hr1 Hr2 Hr3 Hr4 Hr5.

  (* Carry chain telescopes to lo + hi * N_C *)
  assert (Htotal : m0 + m1 * 2^64 + m2 * 2^128 + m3 * 2^192 + m4 * 2^256
    + m5 * 2^320 + c5 * 2^384
  ...
  (* Goal: (lo + hi * 2^256) mod N = lv mod N *)
  f_equal.
  unfold eval8 in Heval.
  lia.
Qed.

(** [reduce_512_stage2_mod] is Stage 2 (385 -> 258 bits): the same fold one
    limb narrower, over the seven limbs [m0..m6] Stage 1 produced.  [mc] is
    the Stage 1 top carry, which the C code reads back as the limb [m6], so
    the two are equated by hypothesis; [p4] is the Stage 2 top carry plus
    [m6]. *)
Lemma reduce_512_stage2_mod :
  forall m0 m1 m2 m3 m4 m5 m6 mc p0 p1 p2 p3 p4
         c0 c1 c2 c3 lv : Z,
  p0 + c0 * 2^64 = m0 + m4 * N_C_0 ->
  p1 + c1 * 2^64 = c0 + m1 + m5 * N_C_0 + m4 * N_C_1 ->
  p2 + c2 * 2^64 = c1 + m2 + m6 * N_C_0 + m5 * N_C_1 + m4 ->
  p3 + c3 * 2^64 = c2 + m3 + m6 * N_C_1 + m5 ->
  m6 = mc ->
  p4 = c3 + m6 ->
  (m0 + m1 * 2^64 + m2 * 2^128 + m3 * 2^192 + m4 * 2^256
   + m5 * 2^320 + mc * 2^384) mod secp256k1_N = lv mod secp256k1_N ->
  (p0 + p1 * 2^64 + p2 * 2^128 + p3 * 2^192 + p4 * 2^256)
    mod secp256k1_N = lv mod secp256k1_N.
Proof.
  intros m0 m1 m2 m3 m4 m5 m6 mc p0 p1 p2 p3 p4
    c0 c1 c2 c3 lv Hr0 Hr1 Hr2 Hr3 Hm6 Hp4 Hprev.
  subst m6.
  subst p4.

  (* Carry chain telescopes to m_lo + m_hi * N_C *)
  assert (Htotal : p0 + p1 * 2^64 + p2 * 2^128 + p3 * 2^192 + (c3 + mc) * 2^256
    = (m0 + m1 * 2^64 + m2 * 2^128 + m3 * 2^192)
      + (m4 + m5 * 2^64 + mc * 2^128)
        * (N_C_0 + N_C_1 * 2^64 + N_C_2 * 2^128)).
  { rep_lia. }

  rewrite Htotal.
  rewrite <- secp256k1_N_C_limbs, fold_sub_mod, <- Hprev.
  f_equal.
  ring.
Qed.

(** [reduce_512_stage3_mod] is Stage 3 (258 -> 256 bits) together with the
    final conditional subtraction.  [c0..c3] are the 128-bit accumulators of
    the four rounds, [rz] the assembled 256-bit result and [ov] the number of
    times [N] must be subtracted (the top carry plus the overflow flag).  The
    conclusion is the postcondition the C code needs: the wrapped sum
    [rz + ov * (2^256 - N)] is [lv mod N]. *)
Lemma reduce_512_stage3_mod :
  forall p0 p1 p2 p3 p4 c0 c1 c2 c3 rz ov lv : Z,
  c0 = p0 + N_C_0 * p4 ->
  c1 = c0 / 2^64 + p1 + N_C_1 * p4 ->
  c2 = c1 / 2^64 + p2 + p4 ->
  c3 = c2 / 2^64 + p3 ->
  0 <= p0 < 2^64 ->
  0 <= p1 < 2^64 ->
  ...
  0 <= p3 < 2^64 ->
  0 <= p4 <= 3 ->
  rz = c0 mod 2^64 + (c1 mod 2^64) * 2^64
       + (c2 mod 2^64) * 2^128 + (c3 mod 2^64) * 2^192 ->
  ov = c3 / 2^64 + (if Z_lt_dec rz secp256k1_N then 0 else 1) ->
  (p0 + p1 * 2^64 + p2 * 2^128 + p3 * 2^192 + p4 * 2^256)
    mod secp256k1_N = lv mod secp256k1_N ->
  (rz + ov * (2^256 - secp256k1_N)) mod 2^256 = lv mod secp256k1_N.
Proof.
  (* Setup: name the hypotheses and inline the four chain definitions *)
  intros p0 p1 p2 p3 p4 c0 c1 c2 c3 rz ov lv
    Hc0 Hc1 Hc2 Hc3 Hp0 Hp1 Hp2 Hp3 Hp4 Hrz Hov Hprev.
  subst c0 c1 c2 c3 rz ov.

  (* Main: bound the folded value, run the carry chain, fold the modulus *)
  (* val + p4 * N_C < 2 * B^4: val < B^4 by eval4_bound, p4 * N_C < B^4 *)
  pose proof (eval4_bound (2^64) p0 p1 p2 p3 ltac:(lia) Hp0 Hp1 Hp2 Hp3)
    as Hval_bnd.

  assert (Hstage3_bound :
    0 <= p0 + p1 * 2^64
        + p2 * (2^64 * 2^64) + p3 * (2^64 * 2^64 * 2^64)
        + p4 * (N_C_0 + N_C_1 * 2^64 + N_C_2 * (2^64 * 2^64))
      < 2 * (2^64 * 2^64 * 2^64 * 2^64)).
  { assert (p4 * (N_C_0 + N_C_1 * 2^64 + N_C_2 * (2^64 * 2^64))
            <= 3 * (N_C_0 + N_C_1 * 2^64 + N_C_2 * (2^64 * 2^64)))
      by (apply Z.mul_le_mono_nonneg_r; rep_lia).
    assert (0 <= p4 * (N_C_0 + N_C_1 * 2^64 + N_C_2 * (2^64 * 2^64)))
      by (apply Z.mul_nonneg_nonneg; rep_lia).
    rep_lia. }

  pose proof (reduce_carry_chain (2^64) p0 p1 p2 p3
    N_C_0 N_C_1 N_C_2 p4
    ltac:(lia) Hp0 Hp1 Hp2 Hp3
  ...
  replace ((2^64)^2) with (2^64*2^64) in Hchain_raw by ring.
  replace ((2^64)^3) with (2^64*2^64*2^64) in Hchain_raw by ring.
  ...
  rewrite (Z.mul_comm p4 N_C_0), (Z.mul_comm p4 N_C_1) in Hchain_raw.
  replace (p4 * 1) with p4 in Hchain_raw by ring.
  ...
  (* fold_sub_mod: val mod N = lv mod N *)
  assert (Hmod_val :
    (p0 + p1 * 2 ^ 64 + p2 * 2 ^ 128 + p3 * 2 ^ 192
     + p4 * (N_C_0 + N_C_1 * 2 ^ 64 + 1 * 2 ^ 128))
    mod secp256k1_N = lv mod secp256k1_N).
  ...
  (* Closeout: (rz + ov * C) mod 2^256 = lv mod N *)
...
(** [reduce_512_ov_bound] is the precondition [secp256k1_scalar_reduce]
    demands of the same Stage 3 chain: each accumulator's carry-out shrinks
    (2^31, 2^32 - 1, 1, 1), so the top carry is at most 1 and the subtraction
    count -- that carry plus the overflow flag -- is at most 2. *)
Lemma reduce_512_ov_bound :
  forall p0 p1 p2 p3 p4 c0 c1 c2 c3 rz : Z,
  c0 = p0 + N_C_0 * p4 ->
  c1 = c0 / 2^64 + p1 + N_C_1 * p4 ->
  c2 = c1 / 2^64 + p2 + p4 ->
  c3 = c2 / 2^64 + p3 ->
  ...
  0 <= p3 < 2^64 ->
  0 <= p4 <= 3 ->
  0 <= c3 / 2^64 + (if Z_lt_dec rz secp256k1_N then 0 else 1) <= 2.
Proof.
  intros p0 p1 p2 p3 p4 c0 c1 c2 c3 rz
    Hc0 Hc1 Hc2 Hc3 Hp0 Hp1 Hp2 Hp3 Hp4.

  assert (Hd0 : 0 <= c0 / 2^64 <= 2147483648).
  { subst c0.
    split.
    - apply Z.div_pos; rep_lia.
    - apply Z.div_le_upper_bound; rep_lia. }
  assert (Hd1 : 0 <= c1 / 2^64 <= 4294967295).
  { subst c1.
    split.
    - apply Z.div_pos; rep_lia.
    - apply Z.div_le_upper_bound; rep_lia. }
  assert (Hd2 : 0 <= c2 / 2^64 <= 1).
  { subst c2.
    split.
    - apply Z.div_pos; rep_lia.
    - apply Z.lt_succ_r, Z.div_lt_upper_bound; rep_lia. }
  assert (Hd3 : 0 <= c3 / 2^64 <= 1).
  { subst c3.
    split.
    - apply Z.div_pos; rep_lia.
    - apply Z.div_le_upper_bound; rep_lia. }

  destruct (Z_lt_dec rz secp256k1_N); lia.
Qed.
...
Lemma body_secp256k1_scalar_reduce_512:
  semax_body Vprog Gprog
    f_secp256k1_scalar_reduce_512 spec_secp256k1_scalar_reduce_512.
Proof.
  start_function.
  ...
  assert (Hcarry_s1_0_ub : acc_val carry_s1_0 <= N_C_0)
    by (eapply reduce_512_carry_ub; [eassumption | rep_lia]).
  ...
  (* m6 = acc.c0 (now uint64_t; no truncating cast) *)
  forward. (* m6 = acc.c0 *)
    ...
    mod secp256k1_N = u512_val l mod secp256k1_N).
  {
    ...
    eapply reduce_512_stage1_mod; eassumption.
  }
  ...
  assert (Hacc_s2_eq : data_at Tsh (t_secp256k1_acc)
      (Vlong (Int64.repr m0v), (Vlong (Int64.repr 0), Vlong (Int64.repr 0))) v_acc__1
    = data_at Tsh (t_secp256k1_acc) (acc_to_val acc_s2_init) v_acc__1)
  ...
  (* muladd(&acc__1, m6, N_C_0) -- m6 is a plain uint64_t *)
  set (m6_val := limb (2^64) (acc_val carry_s1_5) 0).
  assert (Hm6_u64_range : 0 <= m6_val < 2^64).
  { unfold m6_val. rewrite ?limb_at_0. apply Z.mod_pos_bound. lia. }
  ...
  (* muladd_fast(&acc__1, m6, N_C_1) -- m6 is a plain uint64_t *)
  ...
  (* p4 = acc__1.c0 + m6 (now uint64_t; no truncating cast) *)
    ...
    mod secp256k1_N = u512_val l mod secp256k1_N).
  {
    ...
    (* m6 is the Stage 1 top carry; p4 is the Stage 2 top carry plus m6 *)
    assert (Hm6 : m6_val = acc_val carry_s1_5).
    { unfold m6_val.
      rewrite ?limb_at_0.
      rewrite !Zmod_small by rep_lia.
      reflexivity. }
    assert (Hp4v : p4_val = acc_val carry_s2_3 + m6_val).
    { unfold p4_val.
      rewrite ?limb_at_0.
      rewrite Zmod_small by rep_lia.
      reflexivity. }

    eapply reduce_512_stage2_mod; eassumption.
  }

  (* p4 bound needed by reduce_512_stage3_mod -- prove before clearing *)
  ...
  clear - Hsh_r_writable Hsh_l_readable Hfc
          ...
          Hl0 Hl1 Hl2 Hl3 Hn0 Hn1 Hn2 Hn3
          Inh_Scalar
          p0_u64 p1_u64 p2_u64 p3_u64 p4_u64 Hp4_le3
  ...
  forward_call_u128_accum_mul v_c128 c128_0
                ...
                { entailer!. simpl firstn. do 4 f_equal.
    unfold uint64_to_val, p4_u64; simpl u64_val.
    do 2 f_equal.
    unfold p4_val, m6_val; rewrite ?limb_at_0; reflexivity. }
  ...
  (* Core modular arithmetic -- needed for precondition and postcondition:
     reduce_512_stage3_mod folds the Stage 3 carry chain and the conditional
     subtraction, Hstage2_mod connects back to u512_val l.  Both facts below
     stay inside the assert: an equation on the C temps r_z / ov in the
     ambient context would be consumed by the entailer! that follows. *)
  assert (Hmod_eq :
    (r_z + ov * (2^256 - secp256k1_N)) mod 2^256 = u512_val l mod secp256k1_N).
  {
    (* r_z and ov in terms of the round accumulators: lo_i = c_i mod 2^64
       and hi = c3 / 2^64 *)
    assert (Hrz : r_z = u128_val c128_0a mod 2^64
                        + (u128_val c128_1 mod 2^64) * 2^64
                        + (u128_val c128_2 mod 2^64) * 2^128
                        + (u128_val c128_3 mod 2^64) * 2^192).
    { unfold r_z.
      rewrite Hlo0, Hlo1, Hlo2, Hlo3.
      rewrite !u128_lo_val.
      reflexivity. }
    assert (Hov : ov = u128_val c128_3 / 2^64
                       + (if Z_lt_dec r_z secp256k1_N then 0 else 1)).
    { unfold ov, r_u256.
      simpl u256_val.
      rewrite Hhi, u128_hi_val.
      reflexivity. }

    eapply reduce_512_stage3_mod; first [eassumption | rep_lia].
  }
  ...
  { (* 0 <= ov <= 2 *)
    ...
    simpl u256_val.
    rewrite Hhi, u128_hi_val.
    eapply reduce_512_ov_bound; first [eassumption | rep_lia]. }
