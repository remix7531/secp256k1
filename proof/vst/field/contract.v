(** * vst.field.contract: representation predicate + funspecs for the field subsystem. *)
...
(** The field's lazy-reduction discipline lives here: [fe_repr m v ls] says the C
    ...
    [Math.field] value [Fe] is just [v] (the residue); the magnitude [m] and the
...
Require Import secp256k1.vst.helper.structs_field.
...
(* ================================================================= *)
(** ** Field representation: [fe_to_val] and [fe_at]. *)
...
(** Represent the limbs of a [secp256k1_fe] (the [n[5]] array of the one-field
    ...
    [fe_repr] relates the stored limbs to their value and magnitude. *)
...
(* ================================================================= *)
(** ** Field contracts in the arithmetic component. *)
...
(* ================================================================= *)
(** ** [secp256k1_fe_impl_mul]: field multiplication.

    Inputs have magnitudes at most 8 and the result has magnitude 1.
    The limb bounds keep [fe_mul_inner]'s 128-bit accumulators in range. *)

Definition spec_secp256k1_fe_mul : ident * funspec :=
  DECLARE _secp256k1_fe_impl_mul
  WITH r_ptr : val, a_ptr : val, b_ptr : val,
       lr : list Z, la : list Z, lb : list Z,
       ma : Z, mb : Z, va : Z, vb : Z,
       sh_r : share, sh_a : share, sh_b : share
  PRE [ tptr (Tstruct __851 noattr), tptr (Tstruct __851 noattr),
        tptr (Tstruct __851 noattr) ]
    PROP (writable_share sh_r;
          readable_share sh_a;
          readable_share sh_b;
          (0 <= ma <= 8)%Z;
    ...
    SEP (fe_at sh_r r_ptr lr; fe_at sh_a a_ptr la; fe_at sh_b b_ptr lb)
  POST [ tvoid ]
    EX ls : list Z,
    PROP (fe_repr 1 (Z.modulo (Z.mul va vb) secp256k1_P) ls)
    RETURN ()
    SEP (fe_at sh_r r_ptr ls; fe_at sh_a a_ptr la; fe_at sh_b b_ptr lb).

(** Field multiplication contract for the full extraction. *)
Definition Gprog_field_scaffold : funspecs := [
  spec_secp256k1_fe_mul
].
