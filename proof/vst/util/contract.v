(** * vst.util.contract: funspecs for the byte-order helpers in [src/util.h]. *)
...
(** [secp256k1_read_be64] / [secp256k1_write_be64]: big-endian (de)serialization
    ...
    in-scope callers.  The byte<->Z math is [Math.integer_bytes]. *)
...
(* ================================================================= *)
(** ** The module's funspec list -- [src/util.h]'s byte-order and
    secure-clear helpers.  This module has no [static] helpers, so one list
    is the whole module. *)
