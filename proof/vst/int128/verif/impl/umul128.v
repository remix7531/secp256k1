...
(* ================================================================= *)
(** ** Pure-Z helper -- [schoolbook_mul_2x2_mid] (file-local). *)
...
Proof.
  ...
  { subst lo.
    ...
    rewrite <- (low_digits_mod_sq B product) by lia.
    unfold eval2.
    rewrite Hr0, Hr1.
    reflexivity. }
  ...
  { subst hi.
    ...
    subst mid.
    rewrite <- (div_sum_carry_split B (a0*b0/B) (a0*b1) (a1*b0)) by lia.
    fold acc1.
...
(* ================================================================= *)
(** ** secp256k1_umul128 -- [64x64 -> 128 unsigned product]. *)

(** The [Int64.shru]/[Int64.shl] by 32 bridges ([Int64_shru_32],
    [Int64_shl_32]) live in [vst/tactics/core.v]; they are shared with
    the signed [mul128] helper. *)

Lemma body_secp256k1_umul128:
  semax_body Vprog Gprog
    f_secp256k1_umul128 spec_secp256k1_umul128.
Proof.
  start_function.
  ...
  (* ===== Glue: apply schoolbook_mul_2x2_mid ===== *)
