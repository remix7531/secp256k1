(** * contract.modinv: PUBLIC funspecs for the safegcd modular inverse drivers
...
(** This is the public [src/modinv64.h] surface.  The INTERNAL helper funspecs
    ...
    and the modinv proving [Gprog] live in [contract/impl/modinv.v] (which
    re-exports this file).  [Signed62] / [make_modinfo] live in
    [contract/helper/signed62.v] and [debruijn64_array] / [ctz64_*] in
    [contract/util.v]; both are re-exported here since the public specs use them.

    Adapted for Rocq 9.0 / VST 2.16 against this repo's single extraction AST.
    Struct types are anonymous here, so the named-ident references in the original
    are rewritten to the aliases from the structs_* quarantine files. *)
...
(* [Signed62] / [make_modinfo] and [debruijn64_array] are used by the public
   specs; re-export so [contract.impl.modinv] (which re-exports this file) hands
   the whole surface to the verif layer. *)
Require Export secp256k1.contract.helper.signed62.
