(** * contract.impl.modinv: INTERNAL funspecs for the safegcd modular inverse. *)
...
(** Mirrors [src/modinv64_impl.h]: the funspecs here are for the [static]
    ...
    ([secp256k1_modinv64] / [_var]) live in [contract/modinv.v] (re-exported
    here); the modinv proving context is assembled in [contract/gprog/modinv.v].
...
(* Public driver specs ([modinv64] / [_var]) live in [contract.modinv]; re-export
...
Require Export secp256k1.contract.modinv.
...
(* ================================================================= *)
(** ** Helper funspecs -- [from verif/modinv/modinv64.v]. *)

(** The helper-function funspecs were defined inline in the monolithic
    [verif/modinv/modinv64.v].  Moved here so the body proof of each C function
    gets its own verif file (the project one-semax_body-per-file rule).
    ([Trans_repr] now lives in [contract.impl.modinv].)  The modinv proving
    [Gprog] lives in [contract/gprog/modinv.v]. *)

(** [secp256k1_ctz64_var] / [secp256k1_ctz64_var_debruijn] are public-util
    funspecs (both declared in [src/util.h]) -- now in [contract/util.v].  The
    modinv [Gprog] in [contract/gprog/modinv.v] composes them (this file
    requires [contract/util.v], re-exported above). *)
